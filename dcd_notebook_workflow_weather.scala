...
//weather_all
println("weather_all.scala notebook has been started...")
var start_time = getTime(LocalDateTime.now)
val notebook_status = dbutils.notebook.run("weather_all", timeoutSeconds = 0)
var end_time = getTime(LocalDateTime.now)
val duration = start_time.until(end_time, ChronoUnit.SECONDS)
println("status for weather_all.scala notebook : " + notebook_status)
println("elapsed time " + duration + " minutes" )
