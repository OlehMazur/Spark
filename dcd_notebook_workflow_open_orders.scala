...
//oo_orders
println("oo_tuesday_all.scala notebook has been started...")
val start_time = getTime(LocalDateTime.now)
//val notebook_status = dbutils.notebook.run("oo_tuesday_all", timeoutSeconds = 0)
val notebook_status = dbutils.notebook.run("oo_tuesday_all_temporary", timeoutSeconds = 0)
val end_time = getTime(LocalDateTime.now)
val duration = start_time.until(end_time, ChronoUnit.SECONDS)
println("status for oo_tuesday_all.scala notebook : " + notebook_status)
