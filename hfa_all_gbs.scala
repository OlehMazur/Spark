...
where 
calendar_yearmonth >= year(date_add(current_date(),-60))*100 + month(date_add(current_date(),-60)) and
calendar_yearmonth <= year(current_date())*100 + month(current_date())
...
from Calendar 
where DateKey >= year(current_date())*10000 + month(current_date())*100 + day(current_date())
...
where f.calendar_yearweek < ( select distinct WeekId from Calendar where DateKey =  year(current_date())*10000 + month(current_date())*100 + day(current_date()) )
...
and c.WeekId < ( select distinct WeekId from Calendar where DateKey =  year(current_date())*10000 + month(current_date())*100 + day(current_date()) )
...
if (isnull(format), "не определено", format) format, open_orders_promo, open_orders
from 
(
select 
...
and calendar.WeekId >= (select  WeekId from Calendar where DateKey = year(current_date())*10000 + month(current_date())*100 + day(current_date())) 

and tuesday_info.weekid = (select max(WeekId) from Calendar where WeekId  < ( select distinct WeekId from Calendar where DateKey =  year(current_date())*10000 + month(current_date())*100 + day(current_date()) )) --prev week
...
cross join (select max(WeekId) load_week_if_null from Calendar where WeekId  < ( select distinct WeekId from Calendar where DateKey =  year(current_date())*10000 + month(current_date())*100 + day(current_date()) )) prev_week_info
""")
...
cross join 
(select max(WeekId) load_week_if_null  from Calendar where WeekId  < ( select distinct WeekId from Calendar where DateKey =  year(current_date())*10000 + month(current_date())*100 + day(current_date()) )) as load_week_info
""")
...
val sqldf = spark.sql(
  """
  select distinct
  s.calendar_yearmonth partition_name, s.*,
